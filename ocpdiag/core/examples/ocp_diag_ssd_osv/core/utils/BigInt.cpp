...
/*
    ...
    NOTE: This function does not return the absolute value. ToOcpDiagDiagnosis get the absolute
...
BigInt BigInt::operator+(const BigInt& num) const {
    ...
    short carry = 0, sum;
    // add the two values
    for (long long i = static_cast<long long>(larger.size()) - 1; i >= 0; i--) {
...
BigInt BigInt::operator-(const BigInt& num) const {
    ...
    for (i = static_cast<long long>(larger.size()) - 1; i >= 0; i--) {
