...
from google3.third_party.ocpdiag.core.lib.off_dut_machine_interface import remote
from google3.third_party.ocpdiag.core.lib.off_dut_machine_interface.ssh import remote as ssh_remote

MI_SERVICE_ADDR = flags.DEFINE_string(
    'mi_service_addr',
    '',
    'machine interface service address to talk to, if it is empty, the lib uses'
    ' ssh lib by default.',
    allow_override_cpp=True,
)

REMOTE_SSH_KEY_PATH = flags.DEFINE_string(
    'remote_ssh_key_path',
    '',
...
REMOTE_SSH_TUNNEL_FILE_PATH = flags.DEFINE_string(
    'remote_ssh_tunnel_file_path',
    '',
    'optional: ssh control socket file to share the connection and the'
    ' remote_ssh_tunnel_file_path is empty. Check SSH Multiplexing for more'
    ' details.',
    allow_override_cpp=True,
)
...
def NewConn(node: remote.NodeSpec) -> remote.Conn:
  ...
  return ssh_remote.SSHConn(
      node,
      ssh_key_path=REMOTE_SSH_KEY_PATH.value,
      ssh_tunnel_file_path=REMOTE_SSH_TUNNEL_FILE_PATH.value,
